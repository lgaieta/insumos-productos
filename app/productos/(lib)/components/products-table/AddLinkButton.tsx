...
import { editLinkServerAction } from '@productos/actions/editLinkServerAction';
...
function AddLinkButton(props: { productId: Product['id'] }) {
    ...
    const editLinkAction = async (formData: FormData) => {
        const result = await editLinkServerAction(props.productId, formData);
    ...
    if (!isInput)
        return (
            <Tooltip
                color='foreground'
                placement='bottom'
                content='El sitio donde consultas el precio de este producto (por ejemplo, MercadoLibre)'
            >
                <Button
                    variant='flat'
